...
### 세점 메뉴 리디자인
- 가운데 모달 → 하단 시트(bottom sheet)로 변경
- title prop 추가 — 세트 이름 표시로 어떤 세트인지 분간 가능
- 닫기 버튼 추가

### 잠금 단계 선명도 개선
- 2~8단계 opacity 0.4 제거 → 1단계와 동일한 선명도
- 자물쇠 아이콘만으로 잠금 상태 구분

### 액티비티 섹션 추가
- 상세 페이지에 "액티비티" 섹션 신설 — 플래시카드 (기존 단어 목록+필터 이동)
- "학습 로드맵" 섹션 라벨 추가
- 1단계 "단어 소개" → placeholder (다음 스프린트에서 학습 기능 구현 예정)
