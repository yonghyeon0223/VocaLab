...
**UI**
- 타이틀: "학습 구간이 설정됐어요"
- 바 차트: lv.1~10 평가 결과를 색상으로 표시 (경계 구간 강조 규칙 적용)
  - easy 구간: 레벨 번호가 높은 상위 2개만 초록으로 표시, 나머지 easy는 회색
  - appropriate → 보라
  - hard → 주황
  - alien 구간: 레벨 번호가 가장 낮은 1개만 빨강으로 표시, 나머지 alien은 주황(hard 색상)
  - 평가 없음 → 회색
- 결과 카드 3개 (레벨 번호 색상이 차트 색상 구간과 일치)
  - 처음 만날 때: "lv.{easyLevel} — {수준 레이블}" (초록 #4caf7d)
  - 실전 적용: "lv.{activeLevel} — {수준 레이블}" (보라 #6c63ff)
  - 심화: "lv.{hardLevel} — {수준 레이블}" (노랑 #e8a838)
