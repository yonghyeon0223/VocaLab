...
- 쉼표로 결합하는 건 정말 바꿔 써도 의미가 통하는 경우만.
  예: "옳은, 정확한" (correct의 형용사 뜻) → 하나의 객체
  반례: "감각"과 "의미" (sense) → 서로 대체 불가 → 별도 객체
- 자연스러운 한국어로, 학습자가 바로 이해할 수 있게
- 입력된 텍스트에서 사용된 뜻은 반드시 포함
- 텍스트에서는 X라는 뜻으로 쓰였더라도, 학습자가 알아야 할 다른 주요 뜻이
  있다면 별도 객체로 추가해. 다의어를 놓치지 않는 것이 중요해.
- 품사: noun, verb, adj, adv, phrase 중 하나

[참고] 이 학습자의 영어 수준
아래는 한국 교과서 기준 난이도 체계와 각 레벨의 예문이야.
이 학습자의 수준보다 쉬운 단어는 제외해도 되지만, 수준과 같거나 높은 단어는
모두 포함해. 어려운 단어라도 유저가 직접 판단해서 외울 수 있게 해야 해.
